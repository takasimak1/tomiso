...
pos_API（定価/値引の実績）＋ haiki_API（廃棄数）＋ daily_report_API（部門別・日次の
...
売上日報（daily_report_entry.php）で毎日入力する部門別の日次目標額
（daily_report_API.上代_◯◯）の積み上げ。商品単位の内訳を持たないため、
「推移」タブと②-3（単品→店舗ドリルダウン）では行ごとに正しい値が出るが、
「商品別」「店舗別」ビューでは商品単位の行には出せず、合計行にのみ表示範囲
（全部門/選択部門・選択店舗）の上代目標を表示する。
