...
class fmRESTor
{
     ...
     * @return bool|mixed
     */
    public function uploadFormDataToContainer($id, $containerFieldName, $containerFieldRepetition, $file, $relogin = false)
    {
        $this->setLogRowNumber();
        ...
        // cURL で送信するファイルオブジェクトを作成
        $cfile = new \CURLFile($file["tmp_name"], $file["type"], $file["name"]);
        $param = ['upload' => $cfile];
    
        $request = array(
            "url" => "/fmi/data/v1/databases/" . rawurlencode($this->db) . "/layouts/" . rawurlencode($this->layout) . "/records/" . $id . "/containers/" . rawurlencode($containerFieldName) . "/" . $containerFieldRepetition,
            "method" => "POST",
            "headers" => array(
                "Authorization: Bearer " . $this->token
            )
        ...
        try {
            ...
            if ($this->autorelogin === true && $relogin === false && isset($result["status"]["http_code"]) && $result["status"]["http_code"] === self::ERROR_AUTH_RESPONSE_CODE) {
                return $this->uploadFormDataToContainer($id, $containerFieldName, $containerFieldRepetition, $file, true);
            }
        }
